<x-app-layout>
    <x-slot name="title">{{ isset($barang) ? 'Edit' : 'Tambah' }} Data Produk</x-slot>

    <x-page-header title="{{ isset($barang) ? 'Edit Data Produk' : 'Tambah Data Produk' }}">
        <a href="{{ route('data-produk.index') }}" class="inline-flex items-center gap-2 text-sm font-medium text-gray-600 hover:text-gray-800">
    ...
    <div class="max-w-4xl">
        <x-card>
            <form method="POST" action="{{ isset($barang) ? route('data-produk.update', $barang) : route('data-produk.store') }}"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        <x-input-label for="tipe" value="Tipe Produk" />
                        <x-select id="tipe" name="tipe" class="mt-1" x-model="tipe" :disabled="isset($barang)">
                            <option value="barang">Barang</option>
                            <option value="jasa">Jasa</option>
                        </x-select>
                        @if(isset($barang))
                            <p class="mt-1 text-xs text-gray-500">Tipe tidak dapat diubah setelah dibuat.</p>
                        @endif
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div x-show="tipe === 'barang'" x-cloak>
                ...
                <div x-show="tipe === 'barang'" x-cloak class="border border-gray-200 rounded-lg">
                ...
                <div class="flex items-center justify-between pt-2">
                    ...
                    <div class="flex items-center gap-2">
                        <a href="{{ route('data-produk.index') }}" class="inline-flex items-center px-4 py-2.5 text-sm font-medium text-gray-600 hover:text-gray-800">Batal</a>
                        <x-loading-button>
                            {{ isset($barang) ? 'Simpan Perubahan' : 'Simpan Produk' }}
                        </x-loading-button>
                    </div>
                </div>
                ...
                <x-modal name="tambah-satuan" :show="false" maxWidth="sm">
                    <div class="p-6" x-data="satuanModalForm('{{ str_replace(url('/'), '', route('data-produk.tambah-satuan')) }}', 'satuan')">
                ...
                <x-modal name="tambah-kategori" :show="false" maxWidth="sm">
                    <div class="p-6" x-data="satuanModalForm('{{ str_replace(url('/'), '', route('data-produk.tambah-kategori')) }}', 'kategori')">
                ...
                {{-- Scanner Barcode/QR --}}
                <div x-show="tipe === 'barang'" x-cloak x-data="{ aktif: false, pemindai: null }" class="mb-4" id="scanner-container">
                    <button type="button"
    ...
    @push('scripts')
        <script>
            ...
            function produkForm({ tipe, hargaJual, hargaBeli, hargaAvg, fotoPreview }) {
                return {
                    tipe: tipe || 'barang',
