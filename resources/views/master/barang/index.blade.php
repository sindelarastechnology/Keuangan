<x-app-layout>
    ...
    <x-page-header title="Barang" subtitle="Kelola data barang (produk / bahan baku)">
        <a href="{{ route('barang.create') }}" class="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition">
        ...
        <div class="p-4 border-b border-gray-100 space-y-3">
            <form method="GET" class="flex flex-wrap gap-2 items-end">
                ...
                @if(request()->has('cari') || request()->has('kategori') || request()->has('satuan') || request()->has('stok'))
                    <a href="{{ route('barang.index') }}" class="px-3 py-2 text-sm text-gray-600 hover:text-gray-800">Reset</a>
                @endif
            </form>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    @case('aksi')
                                        <td class="px-4 py-3">
                                            <div class="flex justify-center gap-1">
                                                ...
                                                <a href="{{ route('barang.edit', $b) }}" class="inline-flex items-center justify-center w-11 h-11 text-blue-600 hover:bg-blue-50 rounded-md" aria-label="Edit Barang {{ $b->kode }}">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                                </a>
                                                <form method="POST" action="{{ route('barang.destroy', $b) }}">
                                                    @csrf @method('DELETE')
                                                    <button type="button" aria-label="Hapus Barang {{ $b->kode }}" class="inline-flex items-center justify-center w-11 h-11 text-red-600 hover:bg-red-50 rounded-md"
                                                            onclick="window.dispatchEvent(new CustomEvent('open-modal', { detail: 'hapus-barang-{{ $b->id }}' }))">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                                    </button>
                                                    <x-confirm-dialog name="hapus-barang-{{ $b->id }}" title="Hapus Barang"
                                                                      message="Yakin ingin menghapus barang '{{ $b->nama ?: $b->kode }}'? Tindakan ini tidak dapat dibatalkan." />
                                                </form>
                                            </div>
                                        </td>
                                        @break
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ count($kolomAktif) }}" class="px-4 py-16 text-center text-gray-500">Belum ada barang.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if($barang->hasPages())<div class="px-4 py-3 border-t border-gray-100">{{ $barang->links() }}</div>@endif
    </x-card>

    <x-modal name="konfig-kolom" :show="false" maxWidth="md">
        <form method="POST" action="{{ route('barang.simpan-kolom') }}" class="p-6">
            @csrf
            <h3 class="text-lg font-semibold text-gray-800">Atur Kolom Tabel</h3>
            <p class="text-sm text-gray-500 mt-0.5">Pilih kolom yang ingin ditampilkan pada tabel barang.</p>
