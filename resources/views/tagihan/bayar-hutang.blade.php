<x-app-layout>
    <x-slot name="title">Bayar Hutang</x-slot>

    <x-page-header title="Bayar Hutang" :subtitle="$supplier->nama">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <x-card title="Faktur yang Belum Lunas">
            <div class="p-5">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 text-sm">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @foreach($faktur as $f)
                                    <tr class="{{ $f->id === $selectedFakturId ? 'bg-red-50' : '' }}">
                                        <td class="px-4 py-2.5">
                                            <a href="{{ route('pembelian.show', $f) }}" class="font-medium text-red-500 hover:underline">{{ $f->nomor }}</a>
                                            @if($f->id === $selectedFakturId)
                                                <span class="ml-2 inline-flex px-1.5 py-0.5 text-[10px] font-medium rounded-full bg-red-100 text-red-700">Dipilih</span>
                                            @endif
                                        </td>
                                        <td class="px-4 py-2.5 text-gray-600">{{ formatTanggalSingkat($f->tanggal) }}</td>
                                        <td class="px-4 py-2.5 text-right font-medium">{{ formatRupiah(max(0, (float) $f->sisa_hutang)) }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                ...
                @else
                    <p class="text-sm text-gray-500">Tidak ada faktur yang ter-atribusi lagi. Pembayaran akan mengurangi saldo hutang supplier secara umum (resi dari pembukuan lama).</p>
                @endif
            </div>
        </x-card>
        ...
        <x-card title="Form Pembayaran">
            <div class="p-5">
                <dl class="space-y-2 text-sm mb-4 pb-4 border-b border-gray-100">
                    <div class="flex justify-between"><dt class="text-gray-500">Total Hutang</dt><dd class="font-semibold text-red-500">{{ formatRupiah($sisa) }}</dd></div>
                ...
                <form method="POST" action="{{ route('tagihan.hutang.bayar.submit', $supplier) }}" class="space-y-3">
                    @csrf
                    <div>
                        <label for="rekening_id" class="block text-sm text-gray-600 mb-1">Bayar Dari</label>
                        <select id="rekening_id" name="rekening_id" required class="w-full rounded-lg border-gray-300 text-sm focus:border-red-500 focus:ring-red-500">
                    ...
                    <div>
                        ...
                        <input id="tanggal" name="tanggal" type="date" value="{{ old('tanggal', now()->toDateString()) }}" required class="w-full rounded-lg border-gray-300 text-sm focus:border-red-500 focus:ring-red-500">
                    ...
                    <div>
                        ...
                        <textarea id="keterangan" name="keterangan" rows="2" class="w-full rounded-lg border-gray-300 text-sm focus:border-red-500 focus:ring-red-500" placeholder="Opsional">{{ old('keterangan') }}</textarea>
                        @error('keterangan')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                    </div>
                    <div class="flex items-center gap-2 pt-1">
                        <x-button class="inline-flex items-center gap-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 px-3 py-2 rounded-lg transition">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                            Catat Pembayaran
                        </x-button>
                        <span class="text-xs text-gray-400">Tercatat sebagai Kas Keluar dan dapat dibatalkan dari menu Kas Keluar.</span>
                    </div>
                </form>
            </div>
        </x-card>
