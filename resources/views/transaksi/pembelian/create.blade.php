@php($editing = isset($pembelian))
@php($def = $editData ?? [])

<x-app-layout>
    <x-slot name="title">{{ $editing ? 'Edit Draft Pembelian' : 'Pembelian Baru' }}</x-slot>

    <x-page-header title="{{ $editing ? 'Edit Draft Pembelian' : 'Pembelian Baru' }}" subtitle="{{ $editing ? $pembelian->nomor : 'Catat pembelian barang dari supplier' }}">
    ...
    <form id="form-pembelian" method="POST" action="{{ $editing ? route('pembelian.update', $pembelian) : route('pembelian.store') }}" x-data="pembelianForm(pembelianEdit)" @submit="syncAll()">
        @csrf
            ...
            <x-card>
                <div class="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <x-select id="supplier_id" name="supplier_id" class="mt-1" @change="gantiSupplier()" required>
                            <option value="">Pilih Supplier...</option>
                            @foreach($suppliers as $s)
                                <option value="{{ $s->id }}" {{ (old('supplier_id') ? old('supplier_id') == $s->id : ($def['supplier_id'] ?? $supplierDefaultId) == $s->id) ? 'selected' : '' }}>{{ $s->nama }}</option>
                            @endforeach
                        </x-select>
                    ...
                    <div>
                        ...
                        <x-select id="metode_bayar" name="metode_bayar" class="mt-1" @change="ubahMetodeBayar()" required>
                            ...
                            <option value="kredit" {{ old('metode_bayar', $def['metode_bayar'] ?? '') === 'kredit' ? 'selected' : '' }}>Kredit (Utang)</option>
                        </x-select>
                    </div>
                    ...
                    <div>
                        <x-input-label for="gudang_id" value="Gudang Tujuan" />
                        <x-select id="gudang_id" name="gudang_id" class="mt-1">
                            <option value="">— Ikuti Pengaturan —</option>
                            @foreach($gudangList as $g)
                                <option value="{{ $g->id }}" {{ old('gudang_id', $def['gudang_id'] ?? $gudangPembelianId) == $g->id ? 'selected' : '' }}>{{ $g->nama }}</option>
                            @endforeach
                        </x-select>
                ...
                <x-card>
                    <div class="p-4 border-b border-gray-100">
                        ...
                        <div class="mt-3 relative">
                            ...
                            <input type="text" x-model="qGaleri" placeholder="Cari nama / kode barang..." class="w-full pl-9 pr-3 py-2 border-gray-300 rounded-lg text-sm" x-ref="qGaleri">
                        </div>
                    </div>
                ...
                <x-card>
                    <div class="p-4 border-b border-gray-100 flex items-center justify-between gap-2">
                        <div class="flex items-center gap-2">
                            ...
                            <h3 class="font-semibold text-gray-800">Keranjang Pembelian</h3>
    ...
    <div x-show="preview" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" @keydown.escape.window="preview = false">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto" @click.outside="preview = false">
            <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                <h2 class="font-bold text-gray-800">Preview Pembelian</h2>
    ...
    <script>
        var pembelianEdit = @json($editData ?? null);
        function pembelianForm(initial) {
            ...
            return {
                ...
                supplierId: edit ? String(edit.supplier_id || '') : '',
                ...
                galeri: @json($galeri),
                preview: false,
                init() { var s = document.getElementById('supplier_id'); this.supplierId = s ? s.value : ''; },
                bukaPreview() {
                    this.syncAll();
                ...
                optsBarang(brgId) {
                    if (!brgId) return [];
                    var sup = this.supplierId || (document.getElementById('supplier_id') ? document.getElementById('supplier_id').value : '');
                    return (sup && priceOpts[sup] && priceOpts[sup][brgId]) ? priceOpts[sup][brgId] : [];
                },
                badgeClass(k) {
                ...
                autoHarga(item) {
                    ...
                    var tiers = (sup && hargaMap[sup] && hargaMap[sup][item.barang_id])
                        ? hargaMap[sup][item.barang_id]
                        : null;

                    // Harga dari daftar_harga supplier (tiered) — prefer range tersempit
                    var matched = null;
                    ...
                    // Fallback ke harga_beli barang jika tidak ada di daftar_harga
                    var p = dh;
                    if (p === null || p === undefined) {
                ...
                gantiSupplier() {
                    var sup = document.getElementById('supplier_id') ? document.getElementById('supplier_id').value : '';
                    this.supplierId = sup;
                ...
                tambahGaleri(brgId) {
                    ...
                    this.items.push({ barang_id: brgId, jumlah: 1, diskon: 0, harga_satuan: 0 });
                ...
                syncDiskonTipe() {},
                syncAll() {
                    this.ubahMetodeBayar(); this.syncPajak();
