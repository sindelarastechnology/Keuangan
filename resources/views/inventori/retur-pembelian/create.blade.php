<x-app-layout>
    <x-slot name="title">Retur Pembelian Baru</x-slot>

    <x-page-header title="Retur Pembelian Baru" subtitle="Pilih pembelian yang akan diretur — stok & jurnal dikembalikan otomatis">
    ...
    <form method="POST" action="{{ route('retur-pembelian.store') }}" x-data="returPembelianForm()" @submit.prevent="submitForm()">
        @csrf
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
            <x-card>
                <div class="p-4 space-y-4">
                    ...
                    <div>
                        <x-input-label for="pembelian_id" value="Pilih Pembelian" />
                        <select id="pembelian_id" name="pembelian_id" x-model="transaksiId" @change="pilihTransaksi()"
                            class="mt-1 block w-full border-gray-300 rounded-lg text-sm focus:ring-emerald-500 focus:border-emerald-500">
                            <option value="">— Pilih pembelian yang akan diretur —</option>
                            @foreach ($daftarPembelian as $dh)
                        ...
                        <x-input-error :messages="$errors->get('pembelian_id')" class="mt-1" />
                    </div>
                    <div>
            ...
            <div class="lg:col-span-2">
                <x-card>
                    <div class="p-4 border-b border-gray-100">
                        ...
                        <p class="text-xs text-gray-500 mt-0.5">Item pembelian terpilih dimuat otomatis. Isi jumlah yang diretur pada tiap barang yang diinginkan. Jumlah tidak boleh melebihi sisa yang dapat diretur.</p>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 text-sm">
                            ...
                            <tbody>
                                <template x-if="!transaksi">
                                    <tr>
                                        <td colspan="5" class="px-4 py-10 text-center text-sm text-gray-400">Pilih pembelian terlebih dahulu — item akan muncul di sini.</td>
                                    </tr>
                                </template>
                                ...
                                <template x-for="(item, index) in items" :key="item.pembelian_item_id">
                                    <tr>
                                        ...
                                        <td class="px-2 py-2 text-right">
                                            <input type="hidden" :name="'items['+index+'][pembelian_item_id]'" :value="item.pembelian_item_id">
                                        ...
                                        <td class="px-2 py-2 text-right">
                                            ...
                                            <div class="text-[10px] text-gray-400" x-show="hargaBruto(item) !== harga(item)">dari harga beli <span x-text="formatRupiahUnformatted(hargaBruto(item))"></span></div>
                                        </td>
    ...
    <script>
        @php
            $urlSumberRelatif = str_replace(url('/'), '', route('retur-pembelian.sumber-items', ['pembelian' => '__ID__']));
        @endphp
        function returPembelianForm() {
            var urlSumber = @json($urlSumberRelatif);
            return {
                transaksiId: @json((string) old('pembelian_id')),
                transaksi: null,
                memuatItem: false,
                pesanError: '',
                submitting: false,
                ...
                init() {
                    if (this.transaksiId) {
                        this.transaksi = { id: this.transaksiId, label: 'Pembelian #' + this.transaksiId };
                    ...
                    fetch(urlSumber.replace('__ID__', id))
                        ...
                        .then(data => {
                            ...
                            self.items = data.map(o => ({
                                pembelian_item_id: o.pembelian_item_id,
                                jumlah: null,
                            }));
                ...
                submitForm() {
                    ...
                    if (!this.transaksiId || isi.length === 0) {
                        this.pesanError = 'Pilih pembelian dan isi jumlah minimal satu barang yang akan diretur.';
                        return;
                    }
                    this.items = isi;
                    this.pesanError = '';
                ...
                opt(id) { return this.sumberItems.find(o => String(o.pembelian_item_id) === String(id)); },
                namaBarang(item) { var o = this.opt(item.pembelian_item_id); return o ? o.nama : ''; },
                sisa(item) { var o = this.opt(item.pembelian_item_id); return o ? o.sisa : 0; },
                harga(item) { var o = this.opt(item.pembelian_item_id); return o ? (parseFloat(o.harga) || 0) : 0; },
                hargaBruto(item) { var o = this.opt(item.pembelian_item_id); return o ? (parseFloat(o.harga_satuan) || 0) : 0; },
